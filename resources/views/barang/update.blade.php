...
<div>
    <span class="text-xl font-medium text-gray-600">Edit Barang</span>
</div>
    ...
    <form action="{{ route('barang.update', $barang->id_barang) }}" method="POST">
        ...
        <div class="grid gap-6 mb-6 md:grid-cols-2">
            <div>
                ...
                <input type="text" id="nama" name="nama_barang" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" value="{{ old('nama_barang', $barang->nama_barang) }}" />
                @error('nama_barang')
            ...
            <div>
                ...
                <select id="merk" name="merk" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5">
                    ...
                    <option value="Daikin" {{ old('merk', $barang->merk) == 'Daikin' ? 'selected' : '' }}>Daikin</option>
                    <option value="LG" {{ old('merk', $barang->merk) == 'LG' ? 'selected' : '' }}>LG</option>
                    <option value="Panasonic" {{ old('merk', $barang->merk) == 'Panasonic' ? 'selected' : '' }}>Panasonic</option>
                    <option value="Samsung" {{ old('merk', $barang->merk) == 'Samsung' ? 'selected' : '' }}>Samsung</option>
                    <option value="Sharp" {{ old('merk', $barang->merk) == 'Sharp' ? 'selected' : '' }}>Sharp</option> 
            ...
            <div>
                ...
                <select id="jenis" name="jenis_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5">
                    ...
                    @foreach($jenisBarang as $jenis)
                        <option value="{{ $jenis->id_jenis }}" {{ old('jenis_id', $barang->jenis_id) == $jenis->id_jenis ? 'selected' : '' }}>{{ $jenis->nama_jenis }}</option>
                    @endforeach
                </select>
                @error('jenis_id')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
            ...
            <div>
                ...
                <input type="number" id="harga" name="harga" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" value="{{ old('harga', $barang->harga) }}" />
                @error('harga')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
            <div>
                ...
                <input type="number" id="stok" name="stok" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" value="{{ old('stok', $barang->stok) }}" />
                @error('stok')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
