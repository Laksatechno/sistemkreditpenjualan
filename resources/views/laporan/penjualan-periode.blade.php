...
<div class="mb-8">
    <span class="text-xl font-medium text-gray-900">Laporan Penjualan per Periode</span>
</div>
...
<div class="grid grid-cols-3 items-center mb-4">
    ...
    {{-- Datepciker Periode --}}
    <div> 
        <form action="{{ route('laporan.penjualanPeriode') }}" class="max-w-md mx-auto bg-gray-50 border rounded-lg" method="GET">   
    ...
    <div class="flex justify-end gap-2">
        {{-- Tombol Refresh --}}
        <a href="{{ route('laporan.penjualanPeriode') }}" type="submit" class="text-white bottom-2.5 bg-white hover:bg-gray-50 border border-gray-500 font-medium rounded-lg text-sm px-4 py-2">
        ...
        {{-- Tombol Print --}}
        <a href="{{ route('laporan.cetakPenjualanPeriode', ['start' => request('start'), 'end' => request('end')]) }}" class="text-white bottom-2.5 bg-white hover:bg-green-50 border border-green-500 font-medium rounded-lg text-sm px-4 py-2">
...
@if ($penjualanKredit->isEmpty())
    @if (request('start') && request('end'))
        <hr class="mb-8">
        <div class="text-center">
            <i class="text-gray-900 font-medium">Data penjualan tidak ditemukan pada periode ini</i>
        </div>
    @endif
@else
