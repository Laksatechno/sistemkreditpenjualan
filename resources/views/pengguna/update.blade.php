...
<div>
    <span class="text-xl font-medium text-gray-600">Update User</span>
</div>
    ...
    <form action="{{ route('pengguna.update', $pengguna->user_id) }}" method="POST">
        ...
        <div class="grid gap-6 mb-6 md:grid-cols-2">
            <div>
                ...
                <input type="text" id="name" name="name" value="{{ old('nama_jenis', $pengguna->name) }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" placeholder="Masukkan nama pelanggan" />
                @error('name')
            ...
            <div>
                ...
                <input type="text" id="email" name="email" value="{{ old('email', $pengguna->email) }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" placeholder="Masukkan email"/>
                @error('email')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
            <div>
                ...
                <div class="relative">
                    <input type="password" name="password" id="password" value="{{ old('password', $pengguna->password) }}" placeholder="••••••••" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 pr-10">
            ...
            <div>
                ...
                <select id="role" name="role" class="bg-gray-50 border  border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5">
                    ...
                    <option value="superadmin" {{ old('role', $pengguna->role) == 'superadmin' ? 'selected' : '' }}>Superadmin</option>
                    <option value="manajer" {{ old('role', $pengguna->role) == 'manajer' ? 'selected' : '' }}>Manajer</option>
